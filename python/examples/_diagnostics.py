...
Nothing here computes anything about the mesh. The report is the text exokal's
preprocessor writes, and the degenerate-cell rows are the witnesses of its
metric-degeneracy finding -- the same scan, returned as numbers instead of
formatted into a message, so the hundred of them that a real mesh produces can
...
def write_report(mesh_path, out_dir, degeneracy_percent=0.1):
    ...
    text = mk.diagnostics_report(mesh_path, degeneracy_percent)
