...
Setup build(const Parameters& p, double fault_size, double far_size, double band,
  ...
  // than the band itself: the shear increment peaks at the fault centre instead
  // of at the reservoir edges, and the two slip patches never separate.
  ...
  // And the active stretch of the fault itself, |y| <= b. The slip patches live
  // there and nowhere else, and at 12 m their interiors fragment: facets
  // alternate stick / slip because the cone is met to within a facet's
  // resolution, so one patch is reported as four. Refining the whole 4.8 km
  // fault to fix a 300 m stretch would be the wrong trade; refining the three
  // middle pieces is cheap and is where the answer is.
...
// both components are nonzero, the difference from benchmark 1: the fault
// already carries shear before any depletion, and the depletion adds to it.
//
...
// depleted side's pressure rather than the mean of the two. The Python records
// that the 4TU Sigma_slip curve of Fig. 8 matches the one-sided value and is off
// by mu alpha |dp| / 2 under the mean.
...
PreSlip pre_slip(const Setup& s, const Parameters& p, double depletion) {
  ...
  const std::unique_ptr<CauchyMechanicsModel> model = make_model(s, q, false);
...
// with each stage warm-started from the last, which is what puts the patch
// boundaries where the reference has them rather than merely somewhere
// admissible.
...
Slipped simulate(const Setup& s, const Parameters& p, double depletion,
  ...
  const std::unique_ptr<CauchyMechanicsModel> model = make_model(s, q);
  ...
  // the iterate -- what the projection last proposed -- while the facet dofs are
  // what the mechanics actually solved with, and it is the latter the reference
  // reports. Fig. 8 reaches 0.4 MPa against the dataset on this path, through
  // the same accessor, on the locked problem.
  //
...
//
// The exact set-valued law resolves a near-threshold plateau with an alternating
// active set. Where Sigma_C grazes zero over a stretch, adjacent facets settle
// into stick / at-threshold / stick, so the reaction traction -- and with it the
// slip -- carries a facet-scale, mean-zero oscillation. It does not refine away:
// it is what an exact complementarity condition does on a plateau, and the
// Python reference records the same behaviour.
//
// Averaging adjacent facets removes exactly that mode and nothing else: a
// mean-zero oscillation at the sampling frequency is annihilated by a two-point
// mean, while a resolved feature is untouched to second order.
...
//
// The mechanics is built once for a mesh and a material, and a sweep then moves
// only the load and the contact iterate over it.
//
