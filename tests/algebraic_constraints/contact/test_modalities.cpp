...
// apart.
//
//   Conformal. The mesh and the function space stay connected. A fracture is an
//   algebraic constraint on degrees of freedom that already exist: the traction
//   on the facet -- one unknown, shared by both cofaces -- and the displacements
//   and rotations of the two adjacent cells. The jump is the single assembled
//   constitutive row, whose outward incidences make it a difference without any
//   subtraction being written.
//
...
//   and the jump is trace^+ - trace^-, a genuine subtraction of two independent
//   quantities.
//
...
// and driver.hpp cannot tell, the same property that lets the driver serve
// elasticity and poromechanics alike.
//
// Well-posedness is orthogonal to the choice. A fracture that completely severs
// the domain under pure traction data leaves each piece with a rigid-body null
// mode -- in the conformal form the prescribed dof decouples the two halves, in
// the split form the multiplier is undetermined. Global equilibrium then fixes
// the fault traction and no contact law has anything to decide. A fault segment,
...
MIMETIKA_TEST(cutting_creates_the_interface_as_its_own_stratum) {
  ...
  // side -- while the original survives as the detached interface cell: a
  // lower-dimensional stratum of the same complex.
...
// equilibrium then determines the fault traction.
MIMETIKA_TEST(the_crack_tip_is_not_split_and_the_sides_stay_joined) {
  ...
  // must not be posed on under pure traction data.
