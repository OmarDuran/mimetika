...
// space carries no symmetry constraint, and the rotation gamma is the
// multiplier enforcing it against the rigid rotations. Dropping gamma
// changes the method.
//
...
//
// This is a different discretization, not an option: condensing p does not
...
class MixedElasticityTotalCell {
 public:
  ...
  void operator()(const Stencil& st, const std::vector<T>& a, std::vector<T>& r) const {
    ...
    const auto& c = ops_->compact(st.support);
    const std::size_t D = S.end - S.begin;
    // A diagonal star stays diagonal: one multiply per unknown, no dense M.
    const bool diagonal = !c.diag.empty();
    if (D != (diagonal ? c.diag.size() : c.M.rows())) {
      throw std::invalid_argument(
...
struct MechanicsOptions {
  ...
  // field. A different space, so a different layout.
  bool strong_symmetry{false};
  // Add the rotation-jump facet term. Only diagonal_afw carries the half
  // weights it reads, and the term is inert without them, but a cofacet
  // coupling changes the sparsity pattern so it is not attached unasked.
...
class Mechanics final : public Package {
 public:
  ...
  void attach(exokal::forms::Model& model, const exokal::forms::TermContext&) const override {
    ...
    // the stabilization is a second term rather than a branch in the first:
    // it is evaluated on facets, where the star is evaluated on cells
    if (opt_.rotation_jump) {
      model.add("rotation_jump_facet", exokal::forms::On::all(), exokal::forms::Params{});
    }
  }
